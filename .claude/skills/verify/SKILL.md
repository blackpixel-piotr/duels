...
- Player facing: `st.flags.holdPosition` (forwarded from `GameState.HoldPosition`, see targeting model above) gates whether the stationary player snaps to face the enemy. Engaged (`!holdPosition`) → faces the target every frame; held (walked away, not re-engaged) → keeps whatever facing the walk left it with. The enemy always faces the player when stationary (no hold concept on that side).
- Toon PoC parity gaps (expected, not bugs): no whip rope, no HP erosion, no overhead prayer icons, simplified projectiles.
- Equipment rendering: item ids in `WEAPON_ASSETS`/`ARMOR_ASSETS` (toon.js) get real meshes — `steel_sword` sockets into hand_r (+ Sword_Idle stance), the `ranger_*` set skins modular outfit pieces onto the rig and masks the base body's skin under covered regions. TEST FIGHT force-equips the full PoC loadout. Probe via `st.player.weaponMesh` / `st.player.armorMeshes.length` / `st.player.loco[0].role === 'swordIdle'`. Other item ids stay meshless (animation-only) by design.
