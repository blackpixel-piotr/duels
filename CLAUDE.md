...
- Every screen is landscape-first. No vertical-stack page layouts assuming a tall viewport; every screen auto-rotates to fill the display in landscape when the device is held portrait (the same CSS transform trick combat has always used for `.battle-fs`), never a blocking "rotate your device" prompt. Building a portrait-shaped screen is a defect.
