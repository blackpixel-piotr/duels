...
- Every screen is landscape-first. No vertical-stack page layouts assuming a tall viewport; every non-combat screen blocks portrait with a rotate-device prompt (RotateOverlay) — a CSS-transform auto-rotate trick was tried instead and reverted because it breaks native scrolling for any flowing/stacking content (see design/plans/backlog.md's Distribution item and m2-findings.md's second follow-up). Combat is the sole exception, auto-rotating via its own `.battle-fs` trick since it never scrolls. Building a portrait-shaped screen is a defect.
