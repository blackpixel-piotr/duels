...
- **M1 and M2 (Progression Spine) are complete.** M2 shipped item table completion (T1-T4 shop weapons/armour, Maggot King's rare), Bank, Gold Shop, the equipment stat sheet + compare flow, and the Three.js equipment preview. Backlog batch 1 (design/plans/m2-backlog-resolutions.md) then closed the Rotfang unique, Maggot King's full loot table, the specialty-flask shard mechanism, and the cold-start FTUE — see design/plans/backlog.md's Resolved section and design/plans/m2-findings.md's batch-1 addendum. Batch 1's §10 landscape-everywhere mandate (forcing every screen to landscape) was fully reverted per explicit user instruction after repeated real-device breakage — see the "Screen orientation" rule below and m2-findings.md's third follow-up. Still deferred: T3/T4 special-attack mechanics, shop buyback (see backlog.md). **M2 is closed.** **M3 (Combat Grammar as Systems + Bosses 2–4) shipped**: Hive Matron, Mirrorhide, and Bloodtithe are implemented and fought via a new Boss Roster + Pre-Fight screen (UI bible §6, no invocations panel per the brief); shared systems (Perfect Dodge, punish windows, per-duel arena size) were generalized beyond Maggot King along the way. See /design/plans/m3-plan.md for the plan and /design/plans/m3-findings.md for what actually happened, including a flagged tuning risk (Bloodtithe's bleed) and several PROVISIONAL numbers now tracked in backlog.md. M4+ (invocations, remaining bosses, minigame) has not been started or authorized — do not implement ahead of the current milestone.
...
- **Screen orientation**: combat is always played in landscape — the arena auto-rotates via `.battle-fs`'s CSS transform trick regardless of how the phone is physically held. Every other screen (New Game, Hub, Bag, Bank, Shop, Loadout, Character) renders naturally in whatever orientation the device is actually in — vertical phone gets a vertical menu, horizontal phone gets a horizontal one — with no forced layout, no rotate-device prompt, and no CSS rotation hack. A "landscape-everywhere" mandate for these screens was tried twice (a blocking rotate-prompt, then a CSS auto-rotate) and fully reverted both times per explicit user instruction: forcing menus into landscape made them worse to actually use, especially for playtesting, and the auto-rotate variant broke native scrolling outright (see design/plans/backlog.md's Resolved section and m2-findings.md's third follow-up for the full history). Do not reintroduce either approach without being asked.
