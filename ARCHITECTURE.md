...
| `wwwroot/js/vfx.js` | Renderer-only particle layer (`design/plans/vfx-plan.md`), driven entirely by the semantic `vfxEvents` array `GameState` appends per tick — never touches gameplay state, never decides whether something happened. Wraps a `three.quarks` `BatchedRenderer`; event→effect mapping is data (`wwwroot/data/vfx-manifest.json`), never hardcoded. Per-effect emitters are pooled in small fixed-size round-robin groups (pool size doubles as that effect's own concurrency cap, since `ParticleSystem.restart()` kills that instance's own still-live particles) and a global 300-live-particle budget culls the globally-oldest burst first when exceeded. `vfxQuality` (`'off'\|'low'\|'full'`, now clientPref-backed — see toon.js's row) is wired end-to-end but has no UI control yet. Iteration 1 (movement dust): `entity_moved` → a small earth-tone dust puff at the entity's live rendered position (not the raw sim tile), drifting opposite the movement direction. Per-effect pool size is now data-driven (`row.poolSize`, falling back to the shared `DUST_POOL_SIZE=3` default) rather than one constant for every effect — `dust_puff` overrides it to 8, since continuous movement re-triggers it far more often than a one-shot combat effect (playtest request: "bigger, more often"); the manifest row itself was also bumped (count 5→9, size 0.35→0.55, lifetime 0.4→0.45s). `setDustDebug`/`getDustDebug` (on the object `createVfxSystem` returns, proxied as `api.setDustDebug`/`getDustDebug`) let a live debug panel scale size/count and dial concurrency (an `activeSlots` cursor cap under the pre-built pool, no rebuild) without a page reload — mutates the already-built `ParticleSystem`s' value-generator objects in place (`startSize`, `emissionBursts[0].count`), the same technique `spawnBurst` already used for drift. `maxParticle` per pool is `row.count * 4` (was `* 2`) so a live `countMult` above 1 doesn't silently truncate against the construction-time cap. **Round 2** (playtest report: dust "feels cheap" + "an artifact... not under the player" after round 1's tuning): two real per-particle look fixes, both now data-driven per manifest row (defaulting to every existing row's unchanged prior value) rather than hardcoded — `row.liftForce` (was a shared constant `0.05` `ForceOverLife` Y for every effect; dust raised to `0.55` since `PointEmitter`'s fully-spherical initial velocity was sinking particles visibly below the ground plane) and `row.alphaPeak`/`row.fadeInFrac` (a burst's whole `count` spawns on the same frame, so an instant pop to peak alpha with no ramp read as a flat sprite; dust now fades in over the first 15% of its life). Also switched dust's `colorToken` from `--border` (too dark against the ground, read as a smudge with `NormalBlending`) to `--text`, and dialed the round-1 density bump back down (`poolSize` 8→5, `count` 9→7, `size` 0.55→0.5, `lifetime` 0.45→0.35s) — round 1 had overshot "more often" into multiple large, slow-fading puffs visibly trailing the player's recent path, each individually correctly positioned but collectively reading as stray artifacts once the player had moved on. A new `_debugDustSlots()` Playwright probe hook (emitter position + live particles' own positions/age/life) is now permanent, added while ruling out an actual position bug during this investigation (there wasn't one — see vfx-findings.md's round 2 for the full writeup, including a headless-rAF-timing red herring worth knowing about for future probes). Texture is a **procedural placeholder** (radial-gradient `CanvasTexture` generated at init) — the brief's Kenney CC0 particle-pack can't be fetched from this sandbox (`kenney.nl`/`itch.io` both blocked by the egress proxy's org policy); see `vfx-findings.md` and `backlog.md`. **Combat feel pass 1**: manifest rows may carry an optional `style` filter — `handleEvents` only fires a pooled effect whose row has no `style` or whose `style` matches the event's own `data.style`, so `attack_swing`/`impact`/`hit_blocked` each resolve to a *different* effect per doctrine style (`slash_arc_{melee,ranged,magic}`, `impact_burst_{melee,ranged,magic}`, `blocked_spark`/`deflect_ward`/`shield_dome`) instead of one fixed effect per event type; `landing_dust` (`forced_move`) has no style filter, same as `dust_puff`. |
