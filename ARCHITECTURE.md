...
| `wwwroot/index.html` | Minimal HTML. `scrollToBottom`, `saveGame`/`loadGame`/`clearGame`, `triggerShake`, `startMetronome`/`stopMetronome` (tick-cycle sweep + flash, a prayer-flick timing aid), `spawnHitsplats` JS helpers. Registers `service-worker.js`. |
