...
Reads the first model from resources/models/Sakuna/Sukuna Character VOX.vox,
...
def read_first_model(path: str):
    """Return (voxels, palette, size) from first XYZI model in VOX file.

    voxels rows: dict with file coords (fx, fy, fz) and ci.
    """
    ...
    while pos + 12 <= len(buf):
        ...
        start = pos + 12
        if cid == b"SIZE" and first_size is None:
            first_size = struct.unpack("<3i", buf[start : start + 12])
        elif cid == b"XYZI" and first_vox is None:
            n = struct.unpack("<I", buf[start : start + 4])[0]
            first_vox = []
            for i in range(n):
                o = start + 4 + i * 4
                fx, fy, fz, ci = struct.unpack("<4B", buf[o : o + 4])
                first_vox.append({"fx": fx, "fy": fy, "fz": fz, "ci": ci})
        elif cid == b"RGBA":
            palette = [(0, 0, 0)]
            for i in range(255):
                o = start + i * 4
    ...
    if first_vox is None:
        raise ValueError(f"no XYZI chunk found: {path}")
    if palette is None:
        palette = _default_palette()
    return first_vox, palette, first_size
...
def model_bounds_renderer(path: str):
    vox, _palette, _size = read_first_model(path)
...
def build():
    src_vox, src_palette, _size = read_first_model(SOURCE)
    ...
    print("source_voxels", len(src_vox), "colors", len(ci_to_c0), "height", height, "width", width)
