...
(function () {
    ...
    const LAYOUT = {
        player: { wx: 0.0, wz: 3.2,  facing: Math.PI + 0.2, worldH: 2.8 },
        enemy:  { wx: 0.6, wz: -2.6, facing: -0.15,         worldH: 2.6 },
    };
    ...
    // Projected feet position of an actor.
    function anchor(actor, W, H) {
        return worldToScreen(actor.base.wx, actor.base.wz, W, H);
    }
    ...
    function voxelCanvasPos(actor, v, W, H) {
        ...
        const ang = actor.base.facing - CAM_ANGLE;
    ...
    function renderActorOffscreen(actor, flashTint, W, H, angle) {
        const m = actor.model;
        const S = actorScale(actor, W, H);
        ...
        ctx.clearRect(0, 0, w, h);
        renderModel(ctx, actor.view, angle ?? (actor.base.facing - CAM_ANGLE), S, w / 2, h - m.radius * TILT * S - S - 1);
    ...
    function drawActor(ctx, W, H, actor, other, now, windup) {
        ...
        const r = renderActorOffscreen(actor, a.flash, W, H, actor.base.facing - CAM_ANGLE + wobble);
    ...
    function drawWeapon(st, W, H, playerA, now) {
        ...
        const pS = actorScale(pl, W, H);
        // Fit the weapon's larger dimension to ~55% of the player's height —
        // wide flat models (coiled whip) would blow up on height alone.
        const span = Math.max(wp.model.height + 1, wp.model.radius * 2);
        const wS = Math.max(1, Math.floor(m.height * pS * 0.55 / span));
        ...
        const pp = anchor(pl, W, H), pe = anchor(st.enemy, W, H);
        const dirX = Math.sign(pe.x - pp.x) || 1;
        const hx = pp.x + dirX * m.radius * pS * 0.8 + playerA.dx;
        const hy = pp.y - m.height * pS * 0.52 + playerA.dy;
    ...
    async function initBattle(canvasId, opts) {
        ...
        // upscale without aspect distortion. The arena floor is static per
        // size, and the hitsplat zones anchor at the actors' projected upper
        // bodies (splats float up from there) — both recomputed here.
        const zones = [
            [document.getElementById(opts.zonePlayerId), st.player],
            [document.getElementById(opts.zoneNpcId), st.enemy],
        ];
        const fit = () => {
            ...
            for (const [zone, actor] of zones) {
                if (!zone) continue;
                const p = anchor(actor, w, h);
                const top = p.y - actor.model.height * actorScale(actor, w, h) * 0.85;
                zone.style.left = (p.x / w * 100) + '%';
                zone.style.top = (Math.max(0, top) / h * 100) + '%';
            }
        };
        fit();
